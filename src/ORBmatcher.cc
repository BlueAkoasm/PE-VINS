...
/*********************************lcx*****************************/
bool ORBmatcher::inBorder(const cv::Point2f &pt, cv::Mat img)
{
    int Border = 3;
    int width = img.cols;
    int height = img.rows;
    int x = cvRound(pt.x);
    int y = cvRound(pt.y);
    return Border <= x && x < width - Border && Border <= y && y < height - Border;
}
void ORBmatcher::EdgeMatchForTracking(Frame &LastFrame, Frame &CurrentFrame)
{
    // vector<uchar> status;
    vector<float> err;
    if(LastFrame.mValidEdgePixel.size() <= 0) return;
    cv::calcOpticalFlowPyrLK(LastFrame.imgMono, CurrentFrame.imgMono, LastFrame.mValidEdgePixel, CurrentFrame.mMatchedEdgePixel, CurrentFrame.status_e, err, cv::Size(21,21),3);
    for(int i = 0 ; i < CurrentFrame.status_e.size() ; i++)
    {
        if(CurrentFrame.status_e[i] && !inBorder( CurrentFrame.mMatchedEdgePixel[i], CurrentFrame.imgMono))
        {
            // CurrentFrame.status_e[i] = 0;
            // Eigen::Vector3d g1 = LastFrame.GetGradient(LastFrame.mValidEdgePixel[i].x, LastFrame.mValidEdgePixel[i].y);
            // Eigen::Vector3d g2 = CurrentFrame.GetGradient(CurrentFrame.mMatchedEdgePixel[i].x, CurrentFrame.mMatchedEdgePixel[i].y);
            // if(abs(g1(2)-g2(2)) > 45 || !inBorder( CurrentFrame.mMatchedEdgePixel[i], CurrentFrame.imgMono)) 
                CurrentFrame.status_e[i] = 0;
        }
...
void ORBmatcher::EdgeMatchForKeyFrame(KeyFrame* pKF1, KeyFrame* pKF2)
{
    vector<float> err;
    cv::calcOpticalFlowPyrLK(pKF1->imgMono, pKF2->imgMono, pKF1->mValidEdgePixel, pKF2->mMatchedEdgePixel, pKF2->status_e, err, cv::Size(21,21),3);
    for(int i = 0 ; i < pKF2->status_e.size() ; i++)
    {
        if(pKF2->status_e[i] && !inBorder( pKF2->mMatchedEdgePixel[i],pKF2->imgMono))
        {
            // CurrentFrame.status_e[i] = 0;
            // Eigen::Vector3d g1 = LastFrame.GetGradient(LastFrame.mValidEdgePixel[i].x, LastFrame.mValidEdgePixel[i].y);
            // Eigen::Vector3d g2 = CurrentFrame.GetGradient(CurrentFrame.mMatchedEdgePixel[i].x, CurrentFrame.mMatchedEdgePixel[i].y);
