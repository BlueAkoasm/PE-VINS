...
double sumtime2 = 0;
double sumnum2 = 0;
std::ofstream outputFile("/home/linux/project/ORB3_NEW/src/ORB3/results_new/original2_optime.txt",std::ios::trunc);
namespace ORB_SLAM3
{
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose*> vpEdgesPixelMono;
    // vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyPoseToBody *> vpEdgesPixelMono_FHR;
    vector<size_t> vnIndexEdgeMono, vnIndexEdgeRight, vnIndexEdgePixelMono;
    vpEdgesMono.reserve(N);
    vpEdgesMono_FHR.reserve(N);
    vnIndexEdgeMono.reserve(N);
    vnIndexEdgeRight.reserve(N);
    vpEdgesPixelMono.reserve(pFrame->mValidEdgePixel.size());
    vnIndexEdgePixelMono.reserve(pFrame->mValidEdgePixel.size());
    ...
    // {
    //     int num = 0;
    //     int eSize = vnIndexEdgeMono.size();
    //     for(int i=0; i < pFrame->mvpEdgePoints.size(); i++)
    //     {        
    //         EdgePoint* pEP = pFrame->mvpEdgePoints[i];
    //         if(pEP)
    //         {
    //             // nInitialCorrespondences++;
    //             cv::Point kpUn = pFrame->mMatchedEdgePixel[i];

    //             Eigen::Vector2d xj;
    //             xj << kpUn.x, kpUn.y;


    //             // EdgeMonoPixels_lcx* e = new EdgeMonoPixels_lcx(pEP->GetWorldPos(),xj, 0);
    //             // Eigen::Matrix<double,2,1> obs;
    //             // obs << kpUn.x, kpUn.y;
                

    //             ORB_SLAM3::EdgeSE3ProjectXYZPixel* e = new ORB_SLAM3::EdgeSE3ProjectXYZPixel();
    ...
    //             e->pCamera = pFrame->mpCamera;
    //             e->Xw = pEP->GetWorldPos().cast<double>();
    //             e->Xm = xj;
    //             e->pFrame = pFrame;
    //             optimizer.addEdge(e);             
    //             // vpEdgesPixelMono.push_back(e);
    //             // vnIndexEdgePixelMono.push_back(i);
    ...
    //     for(int i=0 ;i < pFrame->mMatchedEdgePixelUn.size(); i++)
    //     {
    //         num++;
    //         EdgePoint* pEP = pFrame->mvpEdgePoints[i];
    //         if(pEP)
    ...
    //                 num1++;
    //                 Eigen::Matrix<double, 2, 1> obs;
    //                 const cv::Point &kpun = pFrame->mMatchedEdgePixelUn[i];
    //                 obs << kpun.x, kpun.y;
    //                 ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose* e = new ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose();
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        // for(size_t i=0, iend=vpEdgesPixelMono.size(); i<iend; i++)
        // {
        //     ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose* e = vpEdgesPixelMono[i];

        //     const size_t idx = vnIndexEdgePixelMono[i];

        //     if(pFrame->mbEdgeOutlier[idx])
        //     {
        //         e->computeError();
        //     }
        ...
        //     if(chi2>chi2Mono[it])
        //     {                
        //         pFrame->mbEdgeOutlier[idx]=true;
        //         e->setLevel(1);
        //         nBad++;
        //     }
        //     else
        //     {
        //         pFrame->mbEdgeOutlier[idx]=false;
        //         e->setLevel(0);
        //     }
...
//     vector<EdgeMonoOnlyPose*> vpEdgesMono;
//     vector<EdgeStereoOnlyPose*> vpEdgesStereo;
//     // vector<EdgeMonoPixels*> vpEdges;
//     vector<EdgeMonoPixels_lcx*> vpEdges;
//     vector<size_t> vnIndexEdgeMono;
//     vector<size_t> vnIndexEdgeStereo;
...
//                     obs << kpUn.pt.x, kpUn.pt.y;
//                     // cout<<obs<<endl;
//                     // Eigen::Vector2f obs1 = pFrame->mpCamera->project(pMP->GetWorldPos());
//                     // Eigen::Vector2d g1 = pFrame->GetGradient2d(obs1(0),obs1(1)).normalized();
//                     // cout<<obs1(0)<<"  "<<obs1(1)<<"|"<<obs(0)<<" "<<obs(1)<<endl;
//                     EdgeMonoOnlyPose* e = new EdgeMonoOnlyPose(pMP->GetWorldPos(),0);
...
//             {
//                 Map* map = pEP->GetMap();
//                 if(!map->GetIniertialBA1()) break;
//                 EdgeCorrespondences++;
//                 cv::Point kpUn = pFrame->mMatchedEdgePixel[i];
//                 // Eigen::Vector2d proj = pFrame->mpCamera->project(pEP->GetWorldPos()).cast<double>();
//                 // Eigen::Vector2d nearestPixel = pFrame->GetNearestPixel((int)proj(0), (int)proj(1));
//                 // cout<<kpUn<<endl;
//                 Eigen::Vector2d xj;
//                 xj << kpUn.x, kpUn.y;

//                 // Eigen::Vector3d g = pFrame->GetGradient(kpUn.x, kpUn.y);

//                 // cout<<kpUn<<"   "<<uv<<endl;


//                 // cv::Point2f p = pFrame->distortPoint(kpUn);
//                 // Eigen::Vector2d g1 = pFrame->GetGradient2d(obs(0),obs(1)).normalized();
//                 // Eigen::Vector2d g2 = pFrame->GetGradient2d(xj(0),xj(1)).normalized();
//                 // cout<<kpUn<<"---  "<<obs2(0)<<" "<<obs2(1)<<endl;
//                 // EdgeMonoPixels* e = new EdgeMonoPixels(pEP->GetWorldPos(), 0);
//                 EdgeMonoPixels_lcx* e = new EdgeMonoPixels_lcx(pEP->GetWorldPos(),xj, 0);
//                 // Eigen::Matrix<double,2,1> obs;
//                 // obs << kpUn.x, kpUn.y;
                

//                 // EdgeMonoOnlyPose* e = new EdgeMonoOnlyPose(pEP->GetWorldPos(),0);
//                 // e->setMeasurement(xj);
//                 // e->setInformation(0.6*Eigen::Matrix<double,2,2>::Identity());

//                 e->setInformation(0.6*Eigen::Matrix<double, 1, 1>::Identity());
//                 e->setVertex(0,VP);
...
//     nInitialCorrespondences = nInitialMonoCorrespondences + nInitialStereoCorrespondences+EdgeCorrespondences;
...
//         {
//             EdgeMonoPixels_lcx* e = vpEdges[i];
...
//             if((chi2>chi2Mono[it])||( chi2>chi2close)||!e->isDepthPositive())
//             {
//                 pFrame->mbEdgeOutlier[idx]=true;
//                 e->setLevel(1);
//                 nBadEdges++;
...
//         nInliers = nInliersMono + nInliersStereo+nInliersEdges;
//         nBad = nBadMono + nBadStereo + nBadEdges;
...
//     }
//     sumnum++;
//     clock_t  optitime  = clock() - start;
//     sumtime+=(double)optitime/CLOCKS_PER_SEC;
//     cout<<"opti time is "<<sumtime/sumnum<<endl;
...
int Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit)
{
    ...
    vector<EdgeStereoOnlyPose*> vpEdgesStereo;
    // vector<EdgeMonoPixels*> vpEdges;
    vector<EdgeMonoOnlyEdge*> vpEdges;
    vector<size_t> vnIndexEdgeMono;
    vector<size_t> vnIndexEdgeStereo;
     vector<size_t> vnIndexEdges;
    vpEdgesMono.reserve(N);
    ...
    /**********************************lcx*******************************/
    {
        ...
        for(int i=0; i < pFrame->mvpEdgePoints.size(); i++)
        {
            ...
            if(pEP)
            {
                ...
                EdgeMonoOnlyEdge* e = new EdgeMonoOnlyEdge(pEP->GetWorldPos(),0);
                e->setMeasurement(xj);
                
                e->setInformation(0.8*Eigen::Matrix<double,2,2>::Identity());

                // e->setInformation(0.8*Eigen::Matrix<double, 1, 1>::Identity());
                e->setVertex(0,VP);
    ...
    sumnum2++;
    clock_t  optitime  = clock() - start;
    double time = static_cast<double>(optitime)/CLOCKS_PER_SEC;
    sumtime2 += time;
    // cout<<"sumtime:"<< sumtime2<<"  sumnum :"<<sumnum2<<"  time:"<<sumtime2/sumnum2<<endl;
    // outputFile << sumtime2/sumnum2<<endl;
    
    // sumtime+=static_cast<double>(optitime)/CLOCKS_PER_SEC;
    // cout<<"opti time is "<<sumtime/sumnum<<endl;
    // std::ofstream outputFile("/home/linux/project/ORB3_NEW/src/ORB3/results_2024/ours_optime.txt",std::ios::trunc);
    // outputFile << sumtime/sumnum<<endl;
    // outputFile.close();
    if ((nInliers<30) && !bRecInit)
    {
        nBad=0;
